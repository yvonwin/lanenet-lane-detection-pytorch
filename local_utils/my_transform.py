...
def init_args():
    ...
    parser.add_argument("--save_dir", type=str, help="Save_path", default="./dataset_new")
...
def skimageFilter(gray):  # 根据标签颜色，标准化标签如:
    ...
    binary_warped[binary_warped > 0.1] = 255  # 注释后取消resize生成的binary_image有问题 为全黑
...
def moveImageTodir(path, targetPath, name):
    if os.path.isdir(path):
        ...
        # train_rows = targetPath+'/'+image_name + " " +targetPath+'/' 
        # +binary_name + " " +targetPath+'/'+instance_name + "\n"+path
...
def gen_train_val_sample(src_dir, save_path, singe=0):
    ...
    if singe:
        ...
        src_list = glob.glob(src_dir + "/*/*_json")
    print(len(src_list))
    trian_data, val_data, test_data = split_data(src_list, 0.8, 0.1, shuffle=True)
    ...
    with open("{:s}/train.txt".format(save_path), "w+") as file:
        for annotations_dir in trian_data:
            ...
            if os.path.isdir(json_dir):
                train_rows = moveImageTodir(json_dir, save_path, str(count).zfill(4))
